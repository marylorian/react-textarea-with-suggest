import { isMobileAndTabletCheck } from "../isMobileAndTabletCheck";

/**
 * Test data from https://deviceatlas.com/blog/list-of-user-agent-strings
 */
...
const userAgentGetter = jest.spyOn(global.navigator, "userAgent", "get");
const navigatorVendorGetter = jest.spyOn(global.navigator, "vendor", "get");

describe("utils/isMobileAndTabletCheck", () => {
  it("should return false if global.navigator and global.window are undefined", () => {
    ...
    expect(isMobileAndTabletCheck()).toBe(false);

    global.window = originalWindow;
    global.navigator = originalNavigator;
  });

  describe("global.navigator.userAgent", () => {
    it.each(mobileUserAgents)(
      "should return true if userAgent contains %s",
      (userAgentMock) => {
        userAgentGetter.mockReturnValueOnce(userAgentMock);

        expect(isMobileAndTabletCheck()).toBe(true);
      }
    );

    it.each(desktopUserAgents)(
      "should return false if userAgent contains %s",
      (userAgentMock) => {
        userAgentGetter.mockReturnValueOnce(userAgentMock);
  ...
  describe("global.navigator.vendor", () => {
    it.each(mobileUserAgents)(
      "should return true if global.navigator.userAgent contains %s",
      (userAgentMock) => {
        userAgentGetter.mockReturnValueOnce("");
        navigatorVendorGetter.mockReturnValueOnce(userAgentMock);
    ...
    it.each(desktopUserAgents)(
      "should return false if global.navigator.userAgent contains %s",
      (userAgentMock) => {
        userAgentGetter.mockReturnValueOnce("");
        navigatorVendorGetter.mockReturnValueOnce(userAgentMock);
